...
Creating keyspaces in cassandra::
    ...
    echo 'CREATE TABLE magnetodb.backup_info(tenant text, table_name text, id uuid, name text, table_name text, status text, start_date_time timestamp, finish_date_time timestamp, location text, strategy map<text, text>, PRIMARY KEY((tenant, table_table), id));' >> ~/.ccm/cql.txt
