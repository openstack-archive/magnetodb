    ...
    def _run_healthcheck(self, context):
        try:
            ...
            if resp.status_code not in [200, 503]:
                ...
                #     API: OK, Keystone: OK, Cassandra: OK, RabbitMQ: OK
                sl = re.split(r'[:,\s]\s*', resp.txt)
                s1 = sl[::2]
                s2 = sl[1::2]
                status = dict(zip(s1, s2))

                identity_ok = status["Keystone"] == "OK"
                storage_ok = status["Cassandra"] == "OK"
                messaging_ok = status["RabbitMQ"] == "OK"
