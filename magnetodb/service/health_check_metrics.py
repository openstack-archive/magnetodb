...
class HealthcheckMetricsService(service.Service, periodic_task.PeriodicTasks):
    ...
    def __init__(self, host='127.0.0.1', port=8480):
        ...
        self.healthcheck_url = (
            'http://' + host + ':' + str(port) + '/healthcheck')
    ...
    def _run_healthcheck(self, context):
        ...
        subsystem_check = health_check.IdentityCheck(
            auth_uri=self.auth_uri)
        try:
            identity_ok = True if subsystem_check.check() else False
        except Exception:
            identity_ok = False

        subsystem_check = health_check.StorageCheck()
        try:
            storage_ok = True if subsystem_check.check() else False
        except Exception:
            storage_ok = False

        subsystem_check = health_check.MessagingCheck()
        try:
            messaging_ok = True if subsystem_check.check() else False
        except Exception:
            messaging_ok = False

        try:
            resp = requests.get(self.healthcheck_url, headers=self.req_headers)
            api_ok = True if resp.status_code == 200 else False
        except Exception:
            api_ok = False

        overall_ok = (True if identity_ok and storage_ok and messaging_ok
                      and api_ok else False)
