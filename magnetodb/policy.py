...
from magnetodb.common.utils import util
...
def init():
    ...
    util.read_cached_file(_POLICY_PATH, _POLICY_CACHE,
...
def enforce(context, action, target, do_raise=True):
    """Verifies that the action is valid on the target in this context.
       ...
       :param context: magnetodb context
       ...
       :raises magnetodb.common.exception.PolicyNotAuthorized: if verification
           fails and do_raise is True.
    ...
    extra = {}
    if do_raise:
        extra.update(exc=exception.PolicyNotAuthorized,
                     message="You are not authorized", action=action)
