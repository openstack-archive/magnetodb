...
class CassandraTableInfoRepository(table_info_repo.TableInfoRepository):
    ...
    def _list_tenants_no_paging(self, limit):
        query_builder = collections.deque()
        query_builder.append("SELECT tenant, name FROM {}".format(
    ...
    def _format_tenant_list(self, table_list):
        result = collections.OrderedDict()
        for row in table_list:
            result.setdefault(row['tenant'], [])
            result[row['tenant']].append(row['name'])
        return result

    @probe.Probe(__name__)
    def list_tenants(self, last_evaluated_project=None, 
                     last_evaluated_table=None, limit=None):
        if last_evaluated_project is None:
            return self._format_tenant_list(
                self._list_tenants_no_paging(limit))

        query_builder = collections.deque()
        query_builder.append("SELECT tenant, name FROM {} "
            "WHERE tenant = '{}'".format(self.SYSTEM_TABLE_TABLE_INFO,
        ...
        query = ("SELECT tenant, name FROM {} WHERE "
        ...
        return self._format_tenant_list(head + tail)
