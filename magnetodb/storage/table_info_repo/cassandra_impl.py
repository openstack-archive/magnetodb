...
class CassandraTableInfoRepository(TableInfoRepository):
    ...
    def __refresh(self, context, table_info, field_list=__field_list):
        ...
        if 'last_updated' not in field_list:
            field_list = tuple(field_list) + ('last_updated',)
    ...
    def update(self, context, table_info, field_list=None):
        ...
        query_builder.append(
            "UPDATE {} SET \"last_updated\"={}, ".format(
                self.SYSTEM_TABLE_TABLE_INFO, table_info.last_updated)
        )
        ...
        query_builder.append(
            ",".join(
                [
                    ...
                    for field in field_list if field != 'last_updated'
                ]
            )
        )
    ...
    def save(self, context, table_info):
        ...
        query_builder.append(
            'INSERT INTO {} '
            '(exists, tenant, name, "schema", status, internal_name, last_updated)'
            "VALUES(1, '{}', '{}'".format(
