...
class CassandraTableInfoRepository(table_info_repo.TableInfoRepository):
    ...
    def _list_tables_no_paging(self, limit):
        query_builder = collections.deque()
    ...
    def _format_table_list(self, table_list):
    ...
    @probe.Probe(__name__)
    def list_tables(self, last_evaluated_project=None, 
                    last_evaluated_table=None, limit=None):
        if last_evaluated_project is None:
            return self._format_table_list(self._list_tables_no_paging(limit))
        ...
        return self._format_table_list(head + tail)
