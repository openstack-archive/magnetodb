...
class CassandraBackupInfoRepository(BackupInfoRepository):
    ...
    def save(self, context, backup_info):
        LOG.debug("Saving backup '{}' for table '{}'".format(
            backup_info.id, backup_info.table_name))

        attrs = {
            name: getattr(backup_info, name)
            for name in self.__set_field_list
        }

        self._update(context, backup_info.table_name, backup_info.id, attrs)
    ...
    def get(self, context, table_name, backup_id):
        ...
        query_builder = collections.deque()
        query_builder.append(
            "SELECT * FROM {} WHERE tenant_table='{}' AND id='{}'".format(
                self.SYSTEM_TABLE_BACKUP_INFO,
                self.__backup_hash(context, table_name), backup_id
            )
        )
    ...
    def list(self, context, table_name,
        ...
        query_builder = collections.deque()
        query_builder.append(
            "SELECT * FROM {} WHERE tenant_table='{}'".format(
                self.SYSTEM_TABLE_BACKUP_INFO,
        ...
        if exclusive_start_backup_id:
            query_builder.append(
                " AND id > '{}'".format(exclusive_start_backup_id)
            )
