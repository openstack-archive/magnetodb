...
def _encode_dynamic_attr_value(attr_value):
    ...
    if collection_type is None:
        ...
        element_type = attr_value.attr_type.element_type
        if not isinstance(val, sortedset):
            val = sortedset(val)
        val = [
        ...
        value_type = attr_value.attr_type.value_type
        res = dict()
        for value, key in val.iteritems():
...
def _storage_to_cassandra__type(storage_type):
...
class CassandraStorageDriver(StorageDriver):
    ...
    def create_table(self, context, table_name):
        ...
        for attr_name, attr_type in (
            ...
            query_builder += (
                ...
                _storage_to_cassandra__type(attr_type), ","
            )
    ...
    def _get_add_attr_value(self, attr_name, attr_value, old_item):
        ...
        if not any((attr_value.is_set, attr_value.is_number)):
            raise InvalidQueryParameter("ADD allows sets or numbers only")
            ...
            local_indexes_conditions = {
                SYSTEM_COLUMN_INDEX_NAME: [
                    models.IndexedCondition.eq(
                        models.AttributeValue('S', index_name)
