...
class CassandraStorageDriver(StorageDriver):
    ...
    def get_counter_item(self, context, table_name, key_attribute_map,
        ...
        query = 'SELECT * FROM "{}{}"."{}"{}'.format(
            USER_PREFIX, context.tenant, cas_counter_table_name,
            where)

        rows = self.__cluster_handler.execute_query(query, consistent)
        if rows:
            return dict(rows[0].iteritems())
        return {}
    ...
    def update_counter_item(self, context, table_name, key_attribute_map,
        ...
        query = ('UPDATE "{}{}"."{}" SET '
                 ...
                 '({{counter_value}}) {}').format(USER_PREFIX, context.tenant,
                                                  cas_counter_table_name,
                                                  where)
        for name, value in counter_attribute_update_map.iteritems():
            self.__cluster_handler.execute_query(query.format(
