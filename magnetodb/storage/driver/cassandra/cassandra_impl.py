...
class CassandraStorageDriver(StorageDriver):
    ...
    def get_table_statistics(self, context, table_info):
        ...
        # TODO(achudnovets): Use the config
        ...
        result = {}
        for item in data:
            if item['request']['mbean'] == metrics['count']['kwargs']['mbean']:
                result['count'] = item['value']
            elif (item['request']['mbean'] ==
                    metrics['size']['kwargs']['mbean']):
                result['size'] = item['value']['Count']
        return result
