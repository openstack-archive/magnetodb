...
DEFAULT_STRING_VALUE = models.AttributeValue('S', '')
DEFAULT_NUMBER_VALUE = models.AttributeValue('N', 0)
...
def _encode_predefined_attr_value(attr_value):
    ...
    if collection_type is None:
        return _encode_single_value_as_predefined_attr(
            attr_value.value, attr_value.attr_type.type
        )
    if collection_type == models.AttributeType.COLLECTION_TYPE_SET:
        ...
        values = ','.join(
            [
                ...
                ) for single_value in attr_value.value
            ]
        )
    ...
    if collection_type == models.AttributeType.COLLECTION_TYPE_MAP:
        ...
        values = ','.join(
            [
                ...
                for key, value in attr_value.value.iteritems()
            ]
        )
        return '{{{}}}'.format(values)
...
def _encode_dynamic_attr_value(attr_value):
    ...
    if collection_type is None:
        val = _encode_single_value_as_dynamic_attr(
            val, attr_value.attr_type.type)
    elif collection_type == models.AttributeType.COLLECTION_TYPE_SET:
        element_type = attr_value.attr_type.element_type
        val = sorted(val)
        val = [
            _encode_single_value_as_dynamic_attr(single_val, element_type)
        ...
        for key, value in val.iteritems():
            res[_encode_single_value_as_dynamic_attr(key, key_type)] = (
                _encode_single_value_as_dynamic_attr(value, value_type)
            )
        val = res
    else:
        assert False
    return "0x{}".format(binascii.hexlify(json.dumps(val, sort_keys=True)))
    ...
    def _get_del_attr_value(self, attr_name, attr_value, old_item):
        ...
        if old_attr_value.is_set:
            ...
            return models.AttributeValue(
                ...
                decoded_value=old_attr_value.value - attr_value.value
            )
        ...
        if old_attr_value.is_map:
            if (not attr_value.is_set or
                ...
                raise InvalidQueryParameter("Wrong type for %s" % attr_name)
            res = old_attr_value.value.copy()
            for key in attr_value.value:
                res.pop(key, None)
            return models.AttributeValue(
    ...
    def _get_add_attr_value(attr_name, attr_value, old_item):
        ...
        if attr_value.is_number:
            return models.AttributeValue(
                ...
                decoded_value=old_item[attr_name].value + attr_value.value
            )

        if attr_value.is_set:
            return models.AttributeValue(
                old_item[attr_name].attr_type,
                decoded_value=old_item[attr_name].value.union(attr_value.value)
            )

        if attr_value.is_map:
            res = old_item[attr_name].value.copy()
            res.update(attr_value.value)
