...
def _encode_and_append_predefined_attr_value(attr_value, query_builder):
    ...
    if attr_type.collection_type:
        ...
        if element_type == models.AttributeType.ELEMENT_TYPE_STRING:
            ...
            query_builder.append("0x{}".format(binascii.hexlify(attr_value.value)))
        else:
            assert False
...
def _encode_dynamic_attr_value(attr_value):
    ...
    return "0x{}".format(binascii.hexlify(json.dumps(attr_value.encoded_value)))
...
def _append_insert_query(table_info, attribute_map, query_builder,
    ...
    query_builder.append(
        'INSERT INTO "{}{}"."{}" ('.format(
            USER_PREFIX, table_info.tenant, table_info.internal_name
        )
    )
...
def _append_delete_query_with_basic_pk(table_info, attribute_map, query_builder):
        query_builder.append(
            'DELETE FROM "{}{}"."{}"'.format(
                USER_PREFIX, table_info.tenant, table_info.internal_name
            )
...
def _append_update_indexes_queries(table_info, old_attribute_map, attribute_map,
                                   query_builder, separator=" ", rewrite=False):
    ...
    def create_base_update_query():
        base_query_builder = (
            _append_update_query_with_basic_pk(
                table_info, attribute_map, collections.deque(), rewrite=rewrite
            )
        )
    ...
    def create_base_delete_query():
        base_query_builder = (
            _append_delete_query_with_basic_pk(
                table_info, attribute_map, collections.deque()
            )
        )
        return "".join(base_query_builder)
...
def generate_put_query(table_info, attribute_map, old_indexes=None,
    ...
    if if_not_exist:
        _append_insert_query(
            table_info, attribute_map, query_builder, if_not_exist=True
        )
    else:
