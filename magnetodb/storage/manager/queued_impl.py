...
from magnetodb.storage.manager.async_simple_impl import AsyncSimpleStorageManager
...
class QueuedStorageManager(AsyncSimpleStorageManager):
    def __init__(self, storage_driver,
                 table_info_repo,
                 concurrent_tasks=1000, batch_chunk_size=25):
        AsyncSimpleStorageManager.__init__(self, storage_driver, table_info_repo,
                                           concurrent_tasks, batch_chunk_size)

        self._rpc_client = SchemaChangeRPCClient()

    def _async_create(self, context, table_info):
        self._rpc_client.create(context.to_dict(), table_info.name)

    def _async_delete(self, context, table_info):
        self._rpc_client.delete(context.to_dict(), table_info.name)
