    ...
    def _validate_table_schema(table_info, attribute_map, keys_only=True,
        ...
        if keys_only and (
            ...
            raise ValidationError(
                ...
                key_attributes=str(attribute_map),
                expected_key_attributes="', '".join(
                    key_attribute_names_to_find
                )
            )
            ...
            if schema_attr_type != typed_attr_value.attr_type:
                raise ValidationError(
                    _("Attribute['%(attr_name)s'] of type['%(attr_type)s'] "
                      "doesn't match table schema expected attribute "
                      "type['%(expected_attr_type)s']"),
                    attr_name=attr_name,
        ...
        if key_attribute_names_to_find:
            raise ValidationError(
                _("Couldn't find expected key "
                  "attributes['%(expected_key_attributes)s']"),
                expected_key_attributes="', '".join(
                    key_attribute_names_to_find
                )
            )
