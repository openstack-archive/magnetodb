...
class SimpleStorageManager(StorageManager):
    ...
    @staticmethod
    def _validate_key_schema(table_info, attribute_map, keys_only=True):
        ...
        if not table_schema_is_valid:
            raise ValidationError(
                _("Specified attribute_map %(key_attribute_map)s "
    ...
    def put_item(self, context, put_request, if_not_exist=False,
        ...
        self._validate_key_schema(table_info, put_request.attribute_map, False)
    ...
    def put_item_async(self, context, put_request, if_not_exist=False,
                       expected_condition_map=None):
        self._validate_key_schema(context, put_request.table_name,
                                  put_request.attribute_map)
        return self._put_item_async(context, put_request, if_not_exist,
                                    expected_condition_map)
    ...
    def delete_item_async(self, context, delete_request,
        ...
        return self._delete_item_async(context, delete_request,
                                       expected_condition_map)
    ...
    def execute_write_batch(self, context, write_request_list):
        ...
        for req in write_request_list:
            def make_request_executor():
                ...
                table_info = self._table_info_repo.get(context, req.table_name)
                self._validate_table_is_active(table_info)

                request_function = None
                if isinstance(_req, PutItemRequest):
                    self._validate_key_schema(table_info, _req.attribute_map,
                                              False)
                    request_function = self._put_item_async
                elif isinstance(_req, DeleteItemRequest):
                    self._validate_key_schema(table_info,
                                              _req.key_attribute_map)
                    request_function = self._delete_item_async
                else:
                    assert False, (
                ...
                def executor():
                    future_result = request_function(context, _req)
    ...
    def execute_get_batch(self, context, read_request_list):
        ...
        for req in read_request_list:
            def make_request_executor():
                ...
                _key_attribute_map = req.key_attribute_map

                table_info = self._table_info_repo.get(context, _table_name)
                self._validate_table_is_active(table_info)
                self._validate_key_schema(table_info, _key_attribute_map)
                ...
                def executor():
                    future_result = self._get_item_async(
                        ...
                        _key_attribute_map.get(
                            table_info.schema.hash_key_name
                        ),
                        _key_attribute_map.get(
                            table_info.schema.range_key_name
                        ),
    ...
    def select_item(self, context, table_name, indexed_condition_map,
        ...
        if len(schema_key_attributes) != 2:
            raise ValidationError(_("Query request is only allowed on tables "
                                    "with RANGE KEY specified"))

        hash_key_name = schema_key_attributes[0]
        range_key_name = schema_key_attributes[1]
        ...
        hash_key_condition_list = None
        range_key_to_query_condition_list = None

        if len(indexed_condition_map) != 2:
            indexed_condition_schema_valid = False
        else:
            hash_key_condition_list = indexed_condition_map.get(hash_key_name,
                                                                None)
            range_key_to_query_condition_list = indexed_condition_map.get(
                range_key_name_to_query, None
            )
            if (hash_key_condition_list is None or
                    range_key_to_query_condition_list is None):
                indexed_condition_schema_valid = False
            else:
                for hash_key_condition in range_key_to_query_condition_list:
                    for hash_key_condition_arg in hash_key_condition.args:
                        if hash_key_condition_arg.attr_type != hash_key_type:
                            indexed_condition_schema_valid = False
                            break

                if indexed_condition_schema_valid:
        ...
        if (len(hash_key_condition_list) != 1 or
            ...
            raise ValidationError(
                _("Only quality condition is allowed for HASH key attribute "
                  "'%(hash_key_name)s'"),
    ...
    def _get_item_async(self, context, table_name, hash_key, range_key,
