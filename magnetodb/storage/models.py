...
class AttributeType(ModelBase):
    ...
    def validate(self, attr_type):
        ...
        if collection_type == self.COLLECTION_TYPE_MAP:
            if len(attr_type) != 3:
                raise ValidationError(self.VALIDATION_ERROR_PATTERN,
                                      type=attr_type)
            key_type = attr_type[1]
            value_type = attr_type[2]
    ...
    @property
    def value_type(self):
        return self.type[2] if len(self.type) == 3 else None
    ...
    def __init__(self, attr_type, encoded_value=None,
        ...
        if encoded_value is not None:
            ...
            if collection_type is None:
                ...
                if isinstance(dict, encoded_value):
                    res_dict = dict()
                ...
                try:
                    value = [
                        self.__decode_single_value(element_type, val)
                        for val in encoded_value
                    ]
                    if collection_type == AttributeType.COLLECTION_TYPE_SET:
