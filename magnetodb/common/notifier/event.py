...
class Notification(object):
    ...
    def __call__(self, **fields):
        ...
        if not set(self.notification_fields).issubset(set(fields.keys())):
            pass

        missing_fields = [o for o in self.notification_fields
                          if o not in fields.keys()]
        if missing_fields:
            raise MissingNotificationFieldsException(
                _('Required field(s) missing in the notification: %s') %
                ', '.join(missing_fields))

        if self.event_type not in self.event_types:
            raise BadEventTypeException(
                _('%s is not a valid event type') % self.event_type)
...
class MissingNotificationFieldsException(Exception):
    pass
