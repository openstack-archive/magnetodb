...
ssl_proxy_middleware_opts = [
    ...
    cfg.StrOpt('secure_proxy_ssl_host_header',
               ...
               help="The HTTP Header that will be used to determine what the "
                    "original request host name/IP address was, even if it was "
                    "removed by an SSL terminator proxy.")
]
...
class SSLProxyMiddleware(wsgi.Middleware):
    ...
    Also, secure_proxy_ssl_host_override is used to override the host name or IP
    address of incoming request, and secure_proxy_ssl_host_header is used to
    configure the host name/IP address http header HTTP_HOST.
