    ...
    def __call__(self, req):
        ...
        if self.statsd_client and self.statsd_client.enabled:
            if is_success(response.status_int):
                ...
                if self.statsd_client.enabled_apiendpoint:
                    event_type = ''
                    ...
                    self._notifier.notify(context, event_type, msg)
            else:
                ...
                if self.statsd_client.enabled_apiendpoint:
                    event_type = ''
                    if hasattr(context, 'event'):
                        event_type = (self.METRICS_FIELD_SEPARATOR_CHAR.join(
                            [context.event,
                             self.ERROR,
                             str(response.status_int)])
                        )
        ...
        if hasattr(context, 'event'):
            context.__delattr__('event')
        if hasattr(context, 'message'):
            context.__delattr__('message')
        return response
