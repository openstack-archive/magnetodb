...
class MagnetoDBNotifier(object):
    ...
    def _deliver_statsd_metrics(self, ctxt, event_metric, payload,
        ...
        if (event_metric and
            ...
            if (event_metric.metric_level ==
                    ...
                    not self.statsd_client.enabled_apiendpoint):
                return
    ...
    def _get_error_event_type(self, event_type):
        """Check if event type is an error event type based on event name.
    ...
    def clean_up_context(self, ctxt):
        """Clean up the following attributes we injected into request context
        object to facilitate message delivery
