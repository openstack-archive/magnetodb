...
class MagnetoDBApplication(wsgi.Router):
    ...
    def __init__(self):
        ...
        mapper.connect(
            ...
            conditions={'method': 'POST'},
            controller=api_v1.create_resource(put_item.PutItemController()),
            action="process_request"
        )
        mapper.connect(
            "get_item", "/{project_id}/tables/{table_name}/get_item",
            conditions={'method': 'POST'},
            controller=api_v1.create_resource(get_item.GetItemController()),
            action="process_request"
        )
        mapper.connect(
            "delete_item",
        ...
        mapper.connect(
            ...
            conditions={'method': 'POST'},
            controller=api_v1.create_resource(scan.ScanController()),
            action="scan"
        )
        mapper.connect(
            "query", "/{project_id}/tables/{table_name}/query",
            conditions={'method': 'POST'},
            controller=api_v1.create_resource(query.QueryController()),
            action="query"
        )
        mapper.connect(
