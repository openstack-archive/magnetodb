...
def bulk_load_app(environ, start_response):
    ...
    LOG.debug('Tenant: %s, table name: %s', tenant_name, table_name)
    ...
    for chunk in stream:
        ...
        try:
            ...
            while put_count - done_count[0] > MAX_FUTURES:
    ...
    while done_count[0] < put_count:
        LOG.debug('Waiting for %d item(s) to be processed...', put_count - done_count[0])
        future_ready_event.wait()
        future_ready_event.clear()
