...
from magnetodb.common.middleware.rate_limit import RateLimitMiddleware
from magnetodb.common.middleware.rate_limit import get_tenant_id
from magnetodb.common.middleware.rate_limit import MDB_DATA_API_URL_PREFIX
...
class TestRateLimit(unittest.TestCase):
    ...
    def test_get_tenant_id(self):
        ...
        path = MDB_DATA_API_URL_PREFIX + expected_tenant_id + "/tables"
        tenant_id = get_tenant_id(path)

        self.assertEqual(expected_tenant_id, tenant_id)

    def setUp(self):
        self.ratelimit_middleware = RateLimitMiddleware(
            None,
            dict(rps_per_tenant=1))
        self.request = Mock()

    def test_process_request_get_tenant_id_from_path(self):
        self.request.path = (MDB_DATA_API_URL_PREFIX +
