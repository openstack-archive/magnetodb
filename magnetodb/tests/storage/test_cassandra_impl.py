...
class TestCassandraBase(unittest.TestCase):
    ...
    test_table_schema = models.TableSchema(
        {
            "id": models.AttributeType('N'),
            "range": models.AttributeType('S'),
            "indexed": models.AttributeType('S'),
            "str": models.AttributeType('S'),
            "numbr": models.AttributeType('N'),
            "blb": models.AttributeType('B'),
            "set_number": models.AttributeType('NS'),
            "set_string": models.AttributeType('SS'),
            "set_blob": models.AttributeType('BS')
        },
        ["id", "range"]
    )
    ...
    test_data_predefined_fields = {
        ...
        'set_number': ('set<decimal>', '{1,2,3}', 'NS', {1, 2, 3}),
        'set_string': ('set<text>', "{'a','b','c'}", 'SS', {'a', 'b', 'c'}),
        'set_blob': (
    ...
    test_data_dynamic_fields = {
        'fnum': ('decimal', binascii.hexlify(json.dumps('1')), 'N', 1),
        'fstr': ('text', binascii.hexlify(json.dumps('fstr')), 'S', 'fstr'),
        'fblb': (
            'blob',
            binascii.hexlify(
        ...
        'fsstr': (
            'set<text>', binascii.hexlify(
                json.dumps(['fa', 'fb', 'fc'])), 'SS', {'fa', 'fb', 'fc'}
        ),
        'fsblob': (
            'set<blob>',
            binascii.hexlify(
    ...
    def tearDownClass(cls):
        ...
        cls.CASANDRA_STORAGE_IMPL = None
    ...
    def test_update_item_put_set_blob(self):
        ...
        actions = {
            'set_blob': models.UpdateItemAction(
                models.UpdateItemAction.UPDATE_ACTION_PUT,
                models.AttributeValue('BS', decoded_value={'new'})),
        }
    ...
    def test_update_item_put_dynamic_set_blob(self):
        ...
        actions = {
            'fsblb': models.UpdateItemAction(
                models.UpdateItemAction.UPDATE_ACTION_PUT,
                models.AttributeValue('BS', decoded_value={'new1', 'new2'})),
        }
...
class TestCassandraPutItem(TestCassandraBase):
    ...
    def test_put_item_expected_dynamic_set_str(self):
        ...
        expected = {
            'fsstr': [
                models.ExpectedCondition.eq(
                    models.AttributeValue('SS',
                                          decoded_value={'fa', 'fb', 'fc'})
                )
            ]
        }
    ...
    def test_put_item_expected_dynamic_set_number(self):
        ...
        put = {
            'id': models.AttributeValue('N', 1),
            'range': models.AttributeValue('S', '1'),
            'fsnum': models.AttributeValue('NS', decoded_value={42, 43}),
        }
        ...
        expected = {
            'fsnum': [
                models.ExpectedCondition.eq(
                    models.AttributeValue('NS', decoded_value={1, 2, 3}))
            ]
        }
