...
cmd_with_request_file = ('table-create', 'item-put', 'item-get', 'item-delete',
                         'query', 'scan', 'batch-write')
...
class MagnetoDBCLIBase(cli.ClientTestBase):
    ...
    def setUpClass(cls):
        ...
        for cmd in cmd_with_request_file:
            file_name = ''.join([cmd, '-request.json'])
            file_path = os.path.join(TEMPLATES_DIR, file_name)
            setattr(cls, ''.join([cmd.replace('-', '_'), '_file']), file_path)
...
class MagnetoDBClientTest(MagnetoDBCLIBase):
    ...
    def _create_table(self):
        params = '--request-file %s' % self.table_create_file
        self.magnetodb('table-create', params=params)
    ...
    def test_magnetodb_create_table(self):
        params = '--request-file %s' % self.table_create_file
        resp = self.magnetodb('table-create', params=params)
        self.assertIn(TABLE_NAME, resp)
        self.assertIn('attribute_definitions', resp)
    ...
    def test_magnetodb_index_describe_non_existent_index(self):
        self._create_table()
        params = '%s %s' % (TABLE_NAME, 'bad-index')
        with self.assertRaises(subprocess.CalledProcessError) as ar:
    ...
    def test_magnetodb_item_put(self):
        self._create_table()
        params = '%s --request-file %s' % (TABLE_NAME, self.item_put_file)
        resp = self.magnetodb('item-put', params=params)
        self.assertIn('hash_attr', resp)

    def test_magnetodb_item_get(self):
        self._create_table()
        params = '%s --request-file %s' % (TABLE_NAME, self.item_put_file)
        self.magnetodb('item-put', params=params)
        params = '%s --request-file %s' % (TABLE_NAME, self.item_get_file)
        resp = self.magnetodb('item-get', params=params)
        self.assertIn('hash_attr', resp)
    ...
    def test_magnetodb_query(self):
        ...
        query_params = '%s --request-file %s' % (TABLE_NAME, self.query_file)
        resp = self.magnetodb('query', params=query_params)
        self.assertEqual('\n', resp)
    ...
    def test_magnetodb_scan(self):
        ...
        scan_params = '%s --request-file %s' % (TABLE_NAME, self.scan_file)
        resp = self.magnetodb('scan', params=scan_params)
        self.assertEqual('\n', resp)
    ...
    def test_magnetodb_batch_write(self):
        ...
        bw_params = '--request-file %s' % self.batch_write_file
        resp = self.magnetodb('batch-write', params=bw_params)
        self.assertEqual('\n', resp)
